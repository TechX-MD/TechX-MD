...
async function connect(phoneNumber) {
    ...
    if (isConnecting) {
        console.log("⚠️ Connection already running:", phoneNumber);
        return;
    }

    isConnecting = true;

    console.log("Starting connection:", phoneNumber);

fs.mkdirSync(`./sessions/${phoneNumber}`, {
    recursive: true
});
...
isConnecting = false;
    ...
    console.log("🗑 Session expired or Linked Device removed.");

    try {
        fs.rmSync(`./sessions/${phoneNumber}`, {
            recursive: true,
            force: true
        });
    ...
    if (global.PAIR_CODES)
        delete global.PAIR_CODES[phoneNumber];

    console.log("♻️ Ready for new pairing.");
...
if (reason === 408) {
    ...
    console.log("⌛ Temporary connection timeout.");
...
sock.ev.on(
    ...
    async ({ messages }) => {
        ...
        // Ignore other broadcast messages
        if (
